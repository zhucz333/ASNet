## 1 ASNet库概述
<br> ASNet(asynchronous-network)库是跨平台的异步网络库，底层兼容Linux平台下的Epoll模型和Windows平台下的IOCP模型，向上提供统一的接口供调用方使用。本库的主要贡献是为网络应用的客户端和服务端提供一套接口封装简单、易用的异步网络库，应用通过回调处理网络事件，同时屏蔽不同平台下网络操作的不同。本库的主要功能包括网络连接的维护、应用数据的读写、`TCP粘包`、异常事件的处理等工作，让应用专注于处理其业务逻辑。<br />
<br> 本库使用CMake编译，首先进入ASNet/src/Build，1）windows平台，运行相关bat脚本，生成vs工程文件后即可编译运行测试样；2）Linux平台，运行相关shell脚本，生成Makefile文件后，即可编译运行测试样例。<br />
### 2 ASNet库介绍
<br>2.1 ASNet库接口类
<br>应用通过该接口来使用本库</br>
```cpp
